#!/usr/bin/env node
/**
 * Recalculate isHotLead + tags on cached LeadInsight rows and webhooks.
...
async function backfillHotLeads() {
    ...
    for (const webhook of webhooks) {
        ...
        if (!tagsChanged && !hotChanged) continue;

        changed += 1;
        if (dryRun) {
            console.log(`[would fix] ${webhook.conversation_id || webhook._id}: hot ${Boolean(existing?.isHotLead)} -> ${insightData.isHotLead}, tags=${JSON.stringify(insightData.tags)}`);
            continue;
        }
    ...
    console.log(`Processed ${webhooks.length} webhooks. Changed: ${changed}. Hot leads: ${hotBefore} -> ${hotAfter}.`);
